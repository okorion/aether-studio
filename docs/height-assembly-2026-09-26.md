...
모니터의 실제 행렬·너비·높이·곡률로 유리 주변의 여유 공간을 계산한다. 꽃 입자는 유리면에 접근할 때 안쪽으로 휘어진다. 꽃이 완성되기 전 진입 전환에도 같은 충돌 방지를 적용한다. 모니터 설정을 바꾸어도 같은 계산을 사용하며, 모니터 뒤의 꽃이 유리에 굴절되어 보이는 표현은 유지한다.
