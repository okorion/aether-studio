...
범위는 **초기 구현부터 `cfee1e1`까지 저장된 코드·리뷰·검증 기록**이다. 같은 원인의 반복 수정은 한 항목에 묶고, 역사 단계와 현재 기준을 구분했다. 이 목록이 미기록 대화·모든 기기·모든 운영 장애를 복원한다는 뜻은 아니다. 이후 변경은 기본 6편의 결과를 바꾸지 않고 별도 부록에서 이어진다.
...
| C02 | 드래그 해제 후 정면 복귀·회전 제한·누르기만 해도 줌 | 결함 | 시작 각도 누적, 각속도만 감쇠, 줌과 복귀 분리. `bdb607d`, [24단계 입력](../continuous-journey.md) |
...
일부 독립 테스트는 모의 renderer·DOM을 사용하고, 일부는 실제 WebGL·영상 디코더로 실행했다. 종류를 바꾸어 설명하지 않는다. 모바일 에뮬레이션도 실물 Safari/iOS의 결과가 아니다. [출처 문서](sources.md)에 원자료 종류를 연결했다.

과거 개인 블로그의 절두체 컬링 사건은 이 색인의 결함 수에 포함하지 않는다. AETHER에서 확인한 내용은 F06의 반사 기여·near-plane·재진입 최적화 방어다. 원본 사이트와의 시각 유사도를 백분율로 측정한 결과도 없다.
