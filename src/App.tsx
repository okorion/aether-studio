...
export default function App() {
  ...
  const ready = loading.status === 'unavailable' || (loading.status === 'ready' && displayPercent === 100)
  ...
  const onLoading = useCallback((stage: LoadingStage) => {
    if (stage === 'module') setDisplayPercent(0)
    setLoading(loadingState(stage))
  }, [])
  ...
  useEffect(() => {
    if (loading.status === 'unavailable' || displayPercent >= loading.percent) return
    // Commit every integer separately, even when several preparation stages
    // finish in one frame. Completion remains gated by the first rendered frame.
    const tick = window.setTimeout(() => setDisplayPercent(value => value + 1), 24)
    return () => window.clearTimeout(tick)
  }, [displayPercent, loading.percent, loading.status])
  useEffect(() => {
  ...
  return (
    <div
      ...
      data-loading-state={ready ? loading.status : 'loading'}
      data-loading-progress={displayPercent}
    >
      ...
      <div className="scene-loading" data-state={ready ? loading.status : 'loading'} hidden={activeSection !== 'home' || loading.status === 'unavailable'}>
        <div className="loading-readout" role="progressbar" aria-label="Scene preparation"
          ...
          aria-valuetext={displayPercent + '% — ' + loading.label}>
          ...
          <span className="loading-number" aria-hidden="true">{String(displayPercent).padStart(3, '0')}<small>%</small></span>
