...
상단에서 하단으로 따라갈 때 시계 방향으로 감기도록 로컬 나선의 방향을 뒤집었다. 본 컬럼의 공통 회전·체인의 하강 속도·반경·기울기·개수·연결 간격은 유지한다. 방향이 바뀐 뒤에도 끝 링크의 구멍이 보이도록 링크 축의 고정 각도를 90도로 맞췄다.

| 대상 | 변경 전 | 변경 후 | 판단 포인트 |
| --- | --- | --- | --- |
| 체인 중간 구간 | ![이전 체인](images/atmosphere-v2/before-chain.jpg) | ![이후 체인](images/atmosphere-v2/after-chain.jpg) | 상단에서 하단으로 시계 방향 감김 |

PC·모바일의 진행률 31%, 46%, 61%에서 같은 카메라·시간 10초·영상 6초로 전후를 캡처했다. 체인 연결·하강·역스크롤 복원·전체 구간 화면 범위 검사 8개와 모니터 가림 관계 검사 1개를 통과했다. 추가 변경 후 lint와 타입 검사를 포함한 production build도 통과했다.
