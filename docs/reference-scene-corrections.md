...
기준 커밋은 `5941628`이다. 사용자 캡처와 [Active Theory](https://activetheory.net/)를 1440×900에서 열어 기존 Aether 화면과 비교했다. 앞부분의 세 요청은 A1~A3, 본 컬럼 이후 요청은 B1~B14로 구분한다. 원본의 코드·영상·모델·텍스처를 프로젝트에 복사하지 않았다.
...
| B8 · 장치 영상 조명 | 숲과 별개 영상, 좁은 구멍에서 퍼지는 투영, 어두운 보조광 | `Scene.tsx`에서 숲과 챔버가 한 영상 디코더를 공유한다. `SceneLighting.ts`, `SceneWorlds.ts`에서 월드 XZ 좌표로 영상 색을 읽고 표면 방향에 따라 반영한다. |
...
| B12 · 하단 숲 드래그 | 숲이 나타난 뒤에도 .94까지 카메라 잠금 | `Journey.ts`에서 .86~.88에 카메라 조작을 복구한다. 숲이 보이는 .89에서 드래그를 검증한다. |
| B13 · 전환면 원형 구멍 | 뿌리·지면 입자를 반경 2.8~3.6 바깥에만 배치하고 화면 중앙에 원형 마스크 적용 | `SceneForest.ts`에서 중앙까지 면적에 비례해 입자를 배치하고 중앙 마스크를 제거한다. 상·하단 숲은 같은 지면 버퍼를 공유한다. |
| B14 · 숲 입자 조립 | 줄기는 실체 메시이고 잎은 고정 위치에서 시간 진동만 수행 | `ForestAssembly.ts`, `SceneForest.ts`에서 줄기·잎 모두 입자로 표현한다. 드문 초기 입자와 숲의 완성 위치를 절대 스크롤 값으로 연결한다. 포인터 유동을 유지하며 역스크롤은 같은 경로를 거꾸로 따른다. |

## 지하 조명의 근거

공개 실행 화면과 [공개 앱 번들](https://activetheory.net/assets/js/app.1780406240914.js), [공개 셰이더](https://activetheory.net/assets/shaders/compiled.vs)를 읽어 확인했다. `ViewController/video`의 공통 reel 입력을 `TreeFBR`, 리액터 입자, `WaterCeilingShader` 등이 공유한다. `TreeFBR`은 월드 XZ 좌표로 영상 색을 읽고 재질 색과 합성한다. 천장도 물무늬와 영상 색을 함께 쓴다. 따라서 척추 위의 구멍에서 내려오는 단일 광선만으로 설명할 수 없다.
...
`lint`, `typecheck`, `build`가 통과했다. 전체 Playwright 실행은 121개 통과·6개 실패·기존 조건부 제외 1개였다. 실패한 검사는 변경 전의 광선 표시·초기 공통 입자·카메라 해제 시점·안개 감쇠를 기대하고 있어 새 동작에 맞춰 수정했다. 관련 40개를 다시 실행해 39개가 통과했고, 초기 숲의 가시 입자 비율을 조정하면서 달라진 새 검사 1개는 기준을 재검토했다. 최종 물살·숲 검사 2개도 통과했다. 최종 커밋 전체 검사는 PR의 CI에서 다시 확인한다.
...
| B13~B14 | 상·하단 전환면의 중앙 입자, 줄기·잎의 점 표현, 스크롤에 따른 숲 조립을 확인. 같은 시간의 정·역방향 11개 구간에서 카메라·모델·회전·체인 진행도 telemetry 차이 0. 숲 위치 표본과 링크 행렬도 정확히 복원. |
...
모두 1440×900에서 같은 진행도·포인터 경로를 사용한 실제 Aether 화면이다. 영상과 실시간 애니메이션의 촬영 시각에는 차이가 있으므로 반사색의 모든 픽셀을 정량 비교하지 않는다. 원본 사이트 캡처는 재배포하지 않는다.
...
| 포레스트 전환면 | ![숲 전환 이전](screenshots/reference-corrections/before/forest-exit.jpg) | ![숲 전환 이후](screenshots/reference-corrections/after/forest-exit.jpg) | 중앙 구멍 제거, 줄기와 잎의 입자 표현 |
