...
**본 컬럼은 Three.js의 `Bone`, `Skeleton`, `SkinnedMesh`를 사용한 리깅을 뜻하지 않습니다.** 현재 구현은 절차적으로 만든 geometry와 `InstancedMesh`의 변환을 스크롤에 맞춰 갱신합니다. 본 컬럼과 체인이라는 이름만으로 골격 애니메이션·관절 물리·체인 물리를 구현했다고 해석하지 않습니다. 리액터도 시각적 장면 이름이며 실제 장치 시뮬레이션을 의미하지 않습니다.
