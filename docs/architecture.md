...
`ScrollTimeline`은 페이지 스크롤과 장면 진행률을 양방향으로 변환한다. 스케일 패널 진입부터 퇴장까지 `.715~.925` 구간에 100svh를 배정한다. `ScaleStage`는 들어오는 패널과 천장을 사선 경계에 맞춰 배치하며 정면 구간의 카메라 방향·거리를 유지한다. 패널 자체 크기는 유지한다. `App`의 본문 상태와 `Scene`의 카메라는 같은 변환을 사용한다. [전환과 배치 변경](scene-framing-2026-09-26.md)에 비교 화면을 정리했다.
