# 포레스트·꽃의 합류 경로와 모니터 설정

후속 변경에서 포레스트 높이와 보강 입자의 시작 높이, 모니터·꽃 반경을 조정했다. 현재 입자 이동과 유리면 간격은 [카메라 높이에 따른 입자 보강](height-assembly-2026-09-26.md), 장면 전환은 [스케일 패널 배치](scene-framing-2026-09-26.md)를 함께 확인한다. 아래 비교와 수치는 당시 기준을 보존한다.
...
포레스트 보강 입자의 약 82%는 각 합류 지점보다 0.22~1.4 위에서 기다린다. 나머지는 1.4~3.2 위에서 들어오며 수평 이동도 줄였다. 처음부터 서 있는 잎·줄기는 유지하고, 위치별로 합류 시점을 조금씩 달리해 포레스트 표면을 보강한다. 하단 포레스트에도 같은 로컬 경로를 적용하므로 천장 쪽으로 합류한다. 완성 상태 유지 구간과 전체 스크롤 길이는 바꾸지 않았다.

텍스트·링 장면의 작은 점은 포레스트의 경계 마스크가 0인데 입자 난수도 0인 경우 남을 수 있었다. 완전히 가려진 입자는 난수와 관계없이 먼저 버린다. GPU 검사에는 난수 0인 입자를 넣어 경계가 닫힐 때 보이지 않고, 역스크롤하면 다시 보이는지 확인한다.
...
모니터 0·1·3·8개의 배치·프로젝트 연결·역스크롤, 서로 다른 모델 비율, 영상 0·1·3개의 생성·해제를 검사한다. 포레스트는 근거리 합류 비율과 닫힌 경계의 실제 GPU 픽셀을 검사하고, 꽃은 실제 정점 셰이더의 중앙 수렴·높이·역방향 위치를 읽는다. 기존 리액터 유체·포인터와 영상 실패 처리 검사를 함께 실행한다.

2026-09-26 로컬 검증에서 lint, typecheck, build와 Playwright 140개 검사가 통과했다. 환경 조건에 따른 1개는 건너뛰었다. 1440×900의 포레스트·꽃 이동 단계와 리액터 입력·복원, 390×844의 주요 장면을 캡처했으며 브라우저 실행 오류는 없었다.
...
| 대상 | 변경 전 | 변경 후 | 판단 포인트 |
| --- | --- | --- | --- |
| 텍스트·링 | ![이전 링 장면](images/scene-flow/text-before.jpg) | ![수정한 링 장면](images/scene-flow/text-after.jpg) | 검은 배경에 남던 포레스트 입자 제거 |
