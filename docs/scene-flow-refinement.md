...
후속 높이 조정에서는 하단 포레스트 전체를 3.0만큼 더 올렸습니다. 현재 천장의 월드 Y는 -48.6입니다. 스크롤 90.5%에서 전환 경계가 화면 위쪽을 빠져나갈 때 천장 입자층도 상단 가까이 놓이며, 아래 기록의 최초 높이 조정 이후에 적용한 변경입니다.
...
- 포레스트 지형은 2D 장면 전환 경계만 바꿔도 월드 위치와 화면 투영이 유지되어야 합니다.
...
모바일의 네이티브 터치로 상단 포레스트·스케일 패널·하단 포레스트를 움직인 검사도 통과했습니다. 드래그 스크롤과 기존 입력 취소 처리를 유지합니다.
