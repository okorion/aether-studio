...
초기 카메라 경로를 비교한 기록이다. 아래 고정 원점·전체 구간 드래그·GPU 영상 표면은 당시 구현을 뜻한다. 현재 하강 경로와 입력·영상 동작은 [구조 문서](architecture.md)에 정리했다.

Active Theory에서 스크롤에 따라 카메라와 장면이 바뀌는 흐름을 관찰하고 경로를 세분화했습니다. 원본 소스·영상·모델은 사용하지 않았습니다. 로고와 작품, 영상 표면은 자체 제작한 절차적 콘텐츠입니다.
