...
PR #8의 MP4 모니터 도입 기록이다. 현재는 브라우저 지원에 따라 WebM 또는 MP4를 고르고, 모니터는 사선 나선을 따라 회전한다. 아래 MP4 전용 상태표와 호버 캡처는 당시 결과다. [영상 자산](media-sources.md) · [후속 배치](scene-reference-detail.md)

[운영 사이트](https://aether-studio-nu.vercel.app/) · [README](../README.md) · [영상 자산](media-sources.md) · [이전 본 컬럼·굴절 구현](spine-monitors.md)

모니터 여섯 장에 직접 제작한 MP4 두 편을 공유해 재생합니다. 얇은 유리판 위에서 영상과 뒤쪽 본 컬럼·입자의 굴절이 함께 보이며, 마우스를 올린 판은 조금 들리고 기울어집니다. 클릭은 기존 프로젝트 상세 화면으로 연결합니다. 본 컬럼 구간의 카메라 드래그 잠금은 유지합니다.

## 원본에서 관찰한 동작

[Active Theory](https://activetheory.net/)를 1440×900·DPR 1의 데스크톱 Chromium에서 실제 휠·마우스로 관찰했습니다. 스크롤을 멈춘 뒤 2.2초 동안 판과 본 컬럼의 위치는 거의 유지되지만 영상 프레임은 바뀌었습니다. 역스크롤로 같은 배열에 돌아와도 영상은 재생을 이어갑니다.

- 정면 판은 거의 평평한 둥근 직사각형이며, 얇은 단면과 틴트 유리의 가장자리가 보입니다.
- 호버에서는 작은 들림·기울기가 관찰됩니다. 영상과 반사도 계속 변하므로 정지 이미지의 밝기 차이 전부를 호버 효과로 해석하지 않습니다.
- 700px 좌우 드래그 중에도 전체 카메라가 본 컬럼 주위를 회전하지 않습니다.
...
판의 배열·본 컬럼·체인은 스크롤을 따르고 영상 재생 시간은 독립적으로 진행합니다. 호버는 마우스가 판 위에 머무는 동안 유지되며 주변 입자 조명의 감쇠와 분리됩니다. 버튼·링크·상세 화면·화면 이탈에서는 호버를 해제합니다. 드래그 중에는 카메라를 잠근 상태로 유지하고 상세 열기도 취소합니다.

같은 판에서 짧게 누르고 놓으면 해당 프로젝트 dialog를 엽니다. 큰 포인터 이동, 스크롤, 다른 판에서의 release, UI 위의 입력은 클릭으로 처리하지 않습니다. 본 컬럼·리액터의 불투명한 면에 가려진 부분도 선택에서 제외합니다. dialog를 열면 영상을 멈추고 닫으면 기존 스크롤 위치에서 재생을 이어갑니다.
...
- 기존 스크롤·카메라 잠금·체인 왕복·굴절 RT의 DPR/상태 복구·접근성 검사를 함께 유지하고, 불투명 구조의 클릭 가림과 해당하지 않는 구조의 제외 조건을 검사합니다.
