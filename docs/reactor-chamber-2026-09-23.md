...
- `Reactor.ts`의 중심과 반경을 공유한다. 개구부 중심은 `(0, -37.25, 0)`, 아래 출구는 y `-37.38`이다. 새 SpotLight는 아래 출구에서 수면 방향을 바라본다. 범위는 빛 원뿔 바닥 가장자리까지의 사선 거리보다 50% 길게 두어 바닥에서 감쇠가 0이 되지 않게 한다. 스케일 패널과 아래층 천장 재질은 이 씬의 유일한 스포트라이트 기여를 제외하고 기존 점광원·방향광은 유지한다. 광선 형상은 개구부 안쪽 반경의 86%에서 시작해 아래로 넓어진다.
...
`tests/chamber.spec.ts`는 실제 SceneWorlds에서 천장·덮개의 중앙 통과, 가장자리·바깥 천장 교차와 광원 위치를 검사한다. 역스크롤 뒤 천장·빛 행렬이 복원되고 자원·광원이 해제되는지도 확인한다. 리뷰에서 광원 범위를 수직 거리로 제한하면 바닥에서 밝기가 0이 된다는 지적을 받아 범위를 늘렸다. 추가 GPU 검사에서 바닥의 실제 밝기 증가, 차단을 적용하지 않은 아래층 대조군의 밝기 증가, 차단한 아래층의 픽셀 변화 0을 확인했다. 실제 수면 shader를 GPU에서 반사 있음/없음으로 렌더링하여 시간 10→12의 픽셀 변화와 10→10의 정지, 12→10의 동일 픽셀 복원을 검사했다. 시간 되감기는 검사 수단이며 사용자 역스크롤에서 수면 시간을 되감도록 구현하지 않았다.

실제 앱에서는 PC·모바일 모두 수동 모션 정지와 운영체제 모션 축소 후 1.8초 간격 canvas PNG의 SHA-256이 같았다. 숨김 이벤트를 주입한 동안 PC·모바일의 renderFrame이 유지되고, 복귀 뒤 다시 증가했다. 이는 앱의 visibility handler 검사이며 실제 운영체제의 탭 스케줄링 검증은 아니다. [PC 기록](evidence/chamber-2026-09-23/desktop-motion.json), [모바일 기록](evidence/chamber-2026-09-23/mobile-motion.json)에 남겼다.
...
6단계는 최신 병합 main에서 모니터 영상·포인터·클릭 후 전환과 복귀를 다룬다. 기존 개구부 좌표와 입자 출발점, 새 천장·빛의 챔버 경계, 영상 비대기 준비 경로를 유지한다. 모니터 캡처를 바꾸면 물 반사가 재귀 렌더되지 않는 경로와 렌더 상태 복원을 다시 확인한다. `excludeChamberSpotlight`는 현재 개구부 광원이 유일한 SpotLight라는 전제로 아래층의 모든 spot directLight를 0으로 만든다. 6~8단계에서 다른 스포트라이트를 추가하면 그 빛도 함께 차단되므로, 해당 시점에 광원별 구분과 아래층 GPU 검사를 다시 설계해야 한다.
