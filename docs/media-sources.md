...
포레스트 곡면 스크린과 공유 조명, 첫 번째 모니터·프로젝트 상세는 `forest-memory.mp4`를 사용한다. 두 번째 모니터는 자체 제작한 `aurora-bloom`을 유지한다. 과도한 크롬 패턴 영상은 런타임에서 제외했다. 아래의 크롬 생성·측정 기록은 과거 자산의 이력이다.

새 순환 영상은 CC0 실사 영상 세 편과 자체 오로라 영상으로 구성한다. 원본 저작자·출처·라이선스·SHA-256, 구간과 색보정, 결과 해시는 [forest-memory manifest](../public/media/forest-memory-manifest.json)에 있다. [재생성 스크립트](../scripts/regenerate-forest-memory.py)는 저장소 밖 원본을 읽으며 다운로드하지 않는다.

- 영상: 512×288, 24fps, 24초, H.264, 무음, 963,587바이트. 576프레임 전체 디코딩과 faststart를 검증한다.
- 포스터: 같은 영상의 2초 프레임인 `forest-memory.jpg`. 모션 축소 상태의 상세 창에서 사용한다.
- 포레스트·조명은 디코더 한 개를 공유한다. 모니터의 별도 디코더는 같은 파일을 선택하며 첫 진입 때만 로드한다.
- 숲 영상은 화면에 붙인 배경이 아니라 월드에 고정된 곡면 메시다. 상단 영상의 아래 가장자리는 바닥보다 0.45 위, 하단 영상의 위 가장자리는 천장보다 0.45 아래에 있다. 하단에서도 영상 방향은 뒤집지 않는다.

```sh
python scripts/regenerate-forest-memory.py --input-directory SOURCE_DIR --output-directory public/media
```
