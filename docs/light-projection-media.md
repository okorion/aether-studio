...
| 항목 | 값 |
| --- | --- |
| 크기 | 275,356 bytes · 상한 400,000 bytes |
| 규격 | 256 × 160, 12 fps, 14초, 168프레임 |
| 포맷 | H.264 Main / yuv420p, 무음, faststart |
| SHA-256 | `bd68b8fdbecf7d8f8ffbc78eb3325641493109f0712315feaa7a427e544c42e3` |
...
렌더 스크립트는 0초부터 14초까지 169장의 PNG를 남깁니다. Vite가 실제로 묶은 모든 모듈, 촬영 스크립트, `package-lock.json`의 해시와 실행 번들 해시를 `source.json` 버전 2에 기록합니다. 현재 입력은 Three.js와 장면 좌표 도우미를 포함해 11개입니다. 촬영 전후 입력이 같아야 검증된 원본으로 저장됩니다.

인코더는 원본 해시·해상도를 확인하고 168프레임을 인코딩한 뒤 입력 해시를 다시 검사합니다. 불완전한 이전 manifest나 변경된 입력은 재촬영을 요구하며 배포 파일로 반영하지 않습니다. `SceneLayers.ts`, lockfile의 불일치와 이전 manifest를 각각 거부하는 동작을 확인했습니다. 전체 디코딩으로 프레임 수·길이·포맷·오디오 부재·faststart·용량을 검증합니다. 마지막→첫 프레임 변화는 전체 인접 프레임 차이의 95백분위와 비교합니다. 0초·14초 원본 렌더는 GPU 연산 차이를 고려해 평균 RGB 오차 0.01/255 이하를 허용합니다.

현재 영상의 디코딩된 RGB 기준 프레임 평균 luma는 3.01–64.18/255입니다. 전체 픽셀 중 luma 24 미만은 81.65%, 150 초과는 7.72%입니다. 최종 WebGL 화면의 선형 광량과는 구분합니다. 이전 사틴 영상의 좁은 밝기 변화보다 명암 차이를 넓혀 동적인 투사가 보이도록 했습니다.
