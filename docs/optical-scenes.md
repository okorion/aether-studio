...
이 문서는 PR #43 당시 구현·측정 기록이다. 이후 물막 효과는 큰 문구의 검은 판으로 한정하고, 링 단면과 본 컬럼 표면을 다시 교정했다. 현재 동작과 비교는 [모바일 광학 교정](mobile-optics-correction.md)을 따른다. 아래 캡처·수치는 당시 결과를 보존한다.
