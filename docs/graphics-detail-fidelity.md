...
아래는 1440×900 실제 화면이다. 본 컬럼 `.405`, 리액터 `.715`, 스케일 패널 `.825`는 모션 축소·시간 0·같은 카메라의 비교다. 글자는 `.175`에서 동일 좌표의 42회 포인터 이동 직후이며, 실시간 프레임의 촬영 시각에는 작은 차이가 있다. 정지 화면으로 유체의 시간 이력을 증명하지 않는다.
