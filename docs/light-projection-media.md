...
| 항목 | 값 |
| --- | --- |
| 크기 | 274,275 bytes · 상한 400,000 bytes |
| 규격 | 256 × 160, 12 fps, 14초, 168프레임 |
| 포맷 | H.264 Main / yuv420p, 무음, faststart |
| SHA-256 | `5ec8f891080b28dc5aa914d1182d3879fcea9f2f7f020b10fd44626121cd0280` |
...
렌더 스크립트는 0초부터 14초까지 169장의 PNG를 남깁니다. Vite가 실제로 묶은 모든 모듈, 촬영 스크립트, `package-lock.json`의 해시와 실행 번들 해시를 `source.json` 버전 3에 기록합니다. 현재 입력은 Three.js와 장면 좌표 도우미를 포함해 11개입니다. 촬영 전후 입력이 같아야 검증된 원본으로 저장됩니다. PNG 169장의 SHA-256도 함께 기록하며, 재촬영을 시작할 때 이전 manifest를 무효화해 촬영 중단 후 프레임이 섞이는 것을 막습니다.
...
현재 영상의 디코딩된 RGB 기준 프레임 평균 luma는 3.01–64.26/255입니다. 전체 픽셀 중 luma 24 미만은 81.66%, 150 초과는 7.72%입니다. 최종 WebGL 화면의 선형 광량과는 구분합니다. 이전 사틴 영상의 좁은 밝기 변화보다 명암 차이를 넓혀 동적인 투사가 보이도록 했습니다.
